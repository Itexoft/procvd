...
public sealed class ProcessConfigResolver
{
        ...
        string baseDirectory)
    {
        ...
        foreach (var processName in processes.Keys.OrderBy(x => x, StringComparer.Ordinal))
        {
            ...
            var outputMaxBytes = outputMode == ProcessOutputMode.File ? settings.OutputMaxBytes ?? defaultOutputMaxBytes : 0;
            var outputMaxFiles = outputMode == ProcessOutputMode.File ? settings.OutputMaxFiles ?? defaultOutputMaxFiles : 0;
            var runAsUser = settings.RunAsUser;
            ...
            if (!string.IsNullOrWhiteSpace(process.Command))
            {
                ...
                var outputPath = outputMode == ProcessOutputMode.File ? ResolveOutputPath(outputDirectory, groupName, processName) : null;
            ...
            var outputPathDirect = outputMode == ProcessOutputMode.File ? ResolveOutputPath(outputDirectory, groupName, processName) : null;
    ...
    private static Dictionary<string, HashSet<string>> BuildGroupMembership(IReadOnlyDictionary<string, HashSet<string>> groupSetMembers)
    {
