...
public sealed class IniProcessConfigLoader(IniReaderOptions? options = null, string? defaultGroupName = null) : IProcessConfigLoader
{
    ...
    {
        var document = IniDocument.Parse(byteStream.ReadAllText(), this.optionsInternal);
    ...
    private static ProcessConfig Parse(IniDocument document, string defaultGroupName)
    {
        ...
        foreach (var section in document.Sections)
        {
            ...
            var groupSectionName = TryParseGroupName(section.Name, out var parsedGroupName) ? parsedGroupName : NormalizeName(section.Name);
    ...
    private static void ApplyProcessField(ProcessBuilder process, string field, IniValueCollection values)
    {
        ...
        if (IsKey(field, "user") || IsKey(field, "run_as") || IsKey(field, "run_as_user") || IsKey(field, "runas"))
        {
            var raw = NormalizeValue(values.FirstOrDefault);

            if (string.IsNullOrWhiteSpace(raw))
                throw new ProcessConfigException($"process '{process.Name}' has empty run_as user");

            process.Settings.RunAsUser = raw;
    ...
    private static bool TryApplySettings(SettingsBuilder settings, string key, IniValueCollection values)
    {
        ...
        if (IsKey(key, "user") || IsKey(key, "run_as") || IsKey(key, "run_as_user") || IsKey(key, "runas"))
        {
            var raw = NormalizeValue(values.FirstOrDefault);

            if (string.IsNullOrWhiteSpace(raw))
                throw new ProcessConfigException($"setting '{key}' expects a non-empty user name");

            settings.RunAsUser = raw;
    ...
    private sealed class SettingsBuilder
    {
        ...
        public ProcessSettings ToSettings()
        {
            if (this.args.Count == 0
                ...
                && string.IsNullOrWhiteSpace(this.RunAsUser)
                && this.OutputMode is null
                && string.IsNullOrWhiteSpace(this.OutputDirectory)
                && this.OutputMaxBytes is null
                && this.OutputMaxFiles is null)
            ...
            return new ProcessSettings
            {
                ...
                RunAsUser = string.IsNullOrWhiteSpace(this.RunAsUser) ? null : this.RunAsUser,
